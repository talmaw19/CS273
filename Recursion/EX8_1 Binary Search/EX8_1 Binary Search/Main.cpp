...
int binary_search(const vector<T> &items, int first, int last, T target) {
	...
	if (items[first] == target)// if the first element of the array equals the last item in the vector 
	{
		return first;// it will return that index
	}
	else {
		binary_search(items, first+1, last, target); 
	}
}
