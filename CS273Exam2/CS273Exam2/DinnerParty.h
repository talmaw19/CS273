...
public:
	...
	*/
	PartyTicket * add(std::string name) {
		...
		*/
		...
		PartyTicket * dPartyTicket = new DinnerPartyTicket(this, it);
	...
	*/
	void list() {
		...
		std::list<std::string> ::iterator itt = room.begin(); // iterator is created to point to everyone that is in the room 
		for (itt; itt != room.end(); itt++)
		{
