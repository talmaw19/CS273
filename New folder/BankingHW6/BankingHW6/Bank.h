...
private:
	...
	*/
	std::vector<int> find_accounts_by_name(std::string name)
	{
		...
		{
			if (Account->get_customer()->getName() == name)
			{
				user_accounts.push_back(Account->get_account());
			}
		}
	...
	*/
		...
		std::string cust_type, std::string account_type)
	{
		...
		// FIXME: Depending on the customer type, we want to create an Adult, Senior, or Student object.
		{
			if (cust_type == "Senior")
			{
				cust = new Senior(age, name, customer_id, address, telephone);
			}
			else if (cust_type == "Adult")
			{
				cust = new Adult(age, name, customer_id, address, telephone);
			}
			else if (cust_type == "Student")
			{
				cust = new Student(age, name, customer_id, address, telephone);
			}
